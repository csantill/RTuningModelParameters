...
```{r initialize,message=FALSE, cache=TRUE}
# load the library
...
pima.data <- data(PimaIndiansDiabetes) # mlbench
...
```{r basetrain, echo=TRUE, cache=TRUE}
reset.seed()
...
tune.gridcart <- expand.grid(maxdepth = 2:8)
...
tune.gridxgb <- expand.grid(eta = c(0.05,0.3, 0.075),
					  nrounds = c(50, 75, 100),
					  max_depth = 4:7,
...
Caret can also simplify model comparision


```{r tunegridxgbthreaded2, cache=TRUE}

model_list <- list(randomgrid=rpartFit2,gridXGB=rpartFit1)
